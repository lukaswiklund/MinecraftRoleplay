 ...
 * their hand. Requires ReflectionUtil class. Built for Minecraft 1.9
 * 
 * @author Jed
 *
 */
public class BookUtils {
	...
	public static Object getItemStack(ItemStack item) {
		try {
			Method asNMSCopy = ReflectionUtils
				.getMethod(PackageType.CRAFTBUKKIT_INVENTORY
	...
	public static void setPages(BookMeta metadata, List<String> pages) {
		...
		try {
			...
			for (String text : pages) {
				page = ReflectionUtils
					.invokeMethod(ReflectionUtils.PackageType.MINECRAFT_SERVER
						.getClass("IChatBaseComponent$ChatSerializer").getDeclaredConstructor().newInstance(),
						"a",
						text
					);
				p.add(page);
